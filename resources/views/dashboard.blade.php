    ...
    <div class="py-12 bg-gray-950 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-12">
                ...
                @if($myVideos->isEmpty())
                    <div class="bg-gray-900/50 border border-dashed border-gray-800 rounded-2xl p-12 text-center">
                        <p class="text-gray-500 italic">Vous n'avez pas encore partagé de tutoriels.</p>
                        <a href="{{ route('videos.create') }}" class="text-indigo-400 text-sm mt-2 inline-block hover:underline">Commencer maintenant</a>
                    </div>
                @else
                        ...
                        @foreach($myVideos as $video)
                            <div class="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 group hover:border-indigo-500/50 transition duration-300 relative">
                                ...
                                <div class="absolute top-3 right-3 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <a href="{{ route('videos.edit', $video->id) }}" class="p-2 bg-blue-600/90 hover:bg-blue-500 text-white rounded-lg shadow-lg backdrop-blur-sm transition">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                    </a>

                                    <form action="{{ route('videos.destroy', $video->id) }}" method="POST" onsubmit="return confirm('Es-tu sûr ?');">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="p-2 bg-red-600/90 hover:bg-red-500 text-white rounded-lg shadow-lg backdrop-blur-sm transition">
                                ...
                                {{-- Image --}}
                                <a href="{{ route('videos.show', $video->slug) }}" class="block relative aspect-video overflow-hidden">
                                    <img src="{{ $video->thumbnail_url }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                                        <span class="text-white text-xs font-bold uppercase border border-white/50 px-3 py-1 rounded">Lire le tuto</span>
                                    </div>
                                </a>
                                ...
                                <div class="p-4 bg-gray-900">
                                    ...
                                    <div class="flex justify-between items-center">
                                        <p class="text-gray-500 text-[10px]">{{ $video->created_at->diffForHumans() }}</p>
                                        <span class="text-[9px] px-2 py-0.5 rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 uppercase font-black">
                                            {{ $video->level }}
                                        </span>
                                    </div>
                                </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    @empty
                        <div class="col-span-full bg-gray-900/50 border border-dashed border-gray-800 rounded-2xl p-12 text-center text-gray-500 italic">
